...
CH340 COM 串口，通过 USB 直接向固件推送状态、额度和旧金山时间；USB 连续失联
5 秒后，固件与应用才使用 WiFi/HTTP 回退。

## 功能

- 左键托盘图标：240×240 设备实时镜像，底部可切换
  自动 / Claude / Codex / Cursor / 时钟，并可调亮度。
...
- Claude/Codex 显示工作状态；Cursor 只监测额度，不检测 Cursor 工作状态。
  Cursor 通常以外圈显示 Total、底部显示 Auto / API；API 剩余显示为 0% 时仅显示 Auto，外圈也切换为 Auto。
- 三家额度独立每 2 分钟刷新。凭据缺失、401/403 或本次启动尚无成功额度时，不进入
  设备轮播；数据超过 15 分钟标记 `STALE`，超过 6 小时移出轮播。
- 时钟页每秒显示 OpenAI 总部所在旧金山的时间、日期和星期，并自动处理夏令时。音乐/Now Playing 功能已删除。
- USB v1 协议支持状态和控制、时钟推送、设备信息、GIF 分块上传、当前桌宠资源读取。
- WiFi 回退服务：`/status`、`/clock`、`POST /event`，监听失败后会自动重试。
